...
class TabManager: NSObject {
    ...
    fileprivate func removeTab(_ tab: Tab, flushToDisk: Bool, notify: Bool) {
        ...
        } else if let newTab = viableTabs.reduce(viableTabs.first, { currentBestTab, t2 in
            if let t1 = currentBestTab, let time1 = t1.lastExecutedTime {
                if let time2 = t2.lastExecutedTime {
                    return time1 <= time2 ? t2 : t1
                }
                return t1
            } else {
                return t2
